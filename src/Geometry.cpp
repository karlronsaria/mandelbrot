...
Functional2D::Functional2D(geo_ptr& geo, unary_t map, flt_t delta) :
	_map(map),
	_delta(delta)
{
	geography(geo);
...
bool Functional2D::contains(int_t x, int_t y) const {
	return abs(coord_y(y) - _map(coord_x(x))) < _delta;
}


Parametric2D::Parametric2D(geo_ptr& geo, unary_t xInverse, unary_t yInverse, flt_t delta) :
	_x_inverse(xInverse),
	_y_inverse(yInverse),
	_delta(delta)
{
...
bool Parametric2D::contains(int_t x, int_t y) const {
	return abs(_y_inverse(coord_y(y)) - _x_inverse(coord_x(x))) < _delta;
}
